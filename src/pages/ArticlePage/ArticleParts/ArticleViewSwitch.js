...
const ArticleViewSwitch = ({ className: parentClassName }) => {
    ...
    //Определяем какие заданы queryParams, в частности display
    ...
    return (
        <div className={clsx(parentClassName, classes.articleViewSwitch)}>
            <ButtonTertiary 
                ...
                onClick={() => followLinkHandler(currentPathname + '')}
                state={ display !== 'on-map' ? 'active': 'non-active' }
            ><DescriptionOutlinedIcon/></ButtonTertiary>

            <ButtonTertiary 
                className={classes.mapButton} 
                onClick={() => followLinkHandler(`${currentPathname}` + '?display=on-map')}
                state={ display === 'on-map' ? 'active': 'non-active' }
